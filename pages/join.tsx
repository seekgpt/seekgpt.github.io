...
// Initialize Supabase client (use your own env vars or hardcoded keys for demo)
...
const MainContent = () => {
	...
	function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) {
		...
		// Map camelCase input names to snake_case in state
		let mappedName = name;
		if (name === 'fullName') mappedName = 'full_name';
		else if (name === 'membershipType') mappedName = 'membership_type';
		setForm((prev) => ({ ...prev, [mappedName]: value }));
	}
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			...
			try {
				...
				// Optionally log or handle email errors, but don't block registration
		...
		<Box>
			<Section size={{ initial: '2', md: '4', lg: '2' }} pb={{ initial: '4', lg: '7' }}>
				<Box style={{ maxWidth: 520 }}>
					<SerifHeading mb="4">Membership Application</SerifHeading>
					<Flex gap="2" mb="4">
						<Box style={{ flex: 1, borderBottom: step === 1 ? '2px solid #6366f1' : '1px solid #e5e7eb', paddingBottom: 4 }}>
							<Text color={step === 1 ? 'indigo' : 'gray'} weight={step === 1 ? 'bold' : undefined}>1. Basic Details</Text>
						</Box>
						<Box style={{ flex: 1, borderBottom: step === 2 ? '2px solid #6366f1' : '1px solid #e5e7eb', paddingBottom: 4 }}>
							<Text color={step === 2 ? 'indigo' : 'gray'} weight={step === 2 ? 'bold' : undefined}>2. Qualifications</Text>
						</Box>
						<Box style={{ flex: 1, borderBottom: step === 3 ? '2px solid #6366f1' : '1px solid #e5e7eb', paddingBottom: 4 }}>
							<Text color={step === 3 ? 'indigo' : 'gray'} weight={step === 3 ? 'bold' : undefined}>3. Mailing & Correspondence</Text>
						</Box>
					</Flex>
					{error && <Box my="2"><Text color="red">{error}</Text></Box>}
					{submitted ? (
						<Box my="4"><Text color="green">Thank you for registering!</Text></Box>
					) : (
						<form onSubmit={step === 3 ? handleSubmit : handleNext}>
							<Flex direction="column" gap="3">
								{step === 1 && (
									<>
										<label>
											<Text as="span">Salutation</Text>
											<select name="salutation" value={form.salutation} onChange={handleChange} required style={{ width: '100%', marginTop: 4, padding: 8 }}>
										   ...
										   <label>
											   ...
											   <input name="fullName" type="text" value={form.full_name} onChange={handleChange} required placeholder="Enter your full name" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										   </label>
										   <label>
											   <Text as="span">Membership Type</Text>
											   <select name="membershipType" value={form.membership_type} onChange={handleChange} required style={{ width: '100%', marginTop: 4, padding: 8 }}>
								...
								{step === 2 && (
									<>
										<label>
											...
											<input name="affiliation" type="text" value={form.affiliation} onChange={handleChange} required placeholder="Your institution or organization" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">Designation</Text>
											<input name="designation" type="text" value={form.designation} onChange={handleChange} required placeholder="Your designation or title" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">Highest Qualification</Text>
											<input name="qualification" type="text" value={form.qualification} onChange={handleChange} required placeholder="Highest qualification" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
									</>
								)}
								{step === 3 && (
									<>
										<label>
											<Text as="span">Corresponding Address</Text>
											<textarea name="address" value={form.address} onChange={handleChange} required rows={3} placeholder="Enter your full address" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">Phone Number</Text>
											<input name="phone" type="tel" value={form.phone} onChange={handleChange} required placeholder="e.g. +1 234 567 8901" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">Email Address</Text>
											<input name="email" type="email" value={form.email} onChange={handleChange} required placeholder="your@email.com" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">State/Province</Text>
											<input name="state" type="text" value={form.state} onChange={handleChange} required placeholder="State or Province" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">Country</Text>
											<input name="country" type="text" value={form.country} onChange={handleChange} required placeholder="Country" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
										<label>
											<Text as="span">PIN/ZIP</Text>
											<input name="pin" type="text" value={form.pin} onChange={handleChange} required placeholder="PIN or ZIP code" style={{ width: '100%', marginTop: 4, padding: 8 }} />
										</label>
									</>
								)}
								<Flex gap="2" mt="2">
									{step > 1 && (
										<Button type="button" variant="soft" color="gray" onClick={handleBack}>Back</Button>
									)}
									{step < 3 && (
										<Button type="submit" color="indigo">Next</Button>
									)}
									{step === 3 && (
										<Button type="submit" color="indigo" disabled={loading}>{loading ? 'Registering...' : 'Register'}</Button>
									)}
								</Flex>
							</Flex>
						</form>
...
const SecondMainContent = () => (
	<Box mt="7" mb="2" style={{ textAlign: 'center' }}>
		<img src="/logos/seekgpt-logo-160x160.png" alt="SeekGPT Logo" style={{ maxWidth: 180, width: '100%', margin: '0 auto' }} />
		<Text as="p" mt="3" color="gray">
			Joining the SeekGPT Foundation will help you stay at the forefront of AI development, connect with a vibrant community of AI practitioners, and contribute to the ethical advancement of artificial general intelligence. We look forward to welcoming you to our community and working together to shape the future of AI.
		</Text>
	</Box>
);
